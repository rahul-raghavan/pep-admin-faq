...
export default function CategoryAssigner({ faqEntryId, currentCategories, allCategories, onUpdate }: CategoryAssignerProps) {
  ...
  return (
    <div>
      <p className="section-heading mb-2">Categories</p>
      <div className="flex flex-wrap items-center gap-1.5">
        {currentCategories.map((cat) => (
          <span key={cat.id} className="inline-flex items-center gap-1 px-2 py-0.5 bg-[#F0EFED] text-[#222]/70 rounded-[4px] text-xs">
            {cat.name}
            <button
              onClick={() => handleRemove(cat.id)}
              disabled={saving}
              className="text-[#222]/40 hover:text-[#222]/60 cursor-pointer"
            >
              &times;
            </button>
          </span>
            ...
            <div className="absolute top-full left-0 mt-1 bg-white border border-[#F0EFED] rounded-[4px] shadow-lg z-10 min-w-[160px]">
              {available.map((cat) => (
                <button
                  ...
                  className="block w-full text-left px-3 py-2 text-sm text-[#222] hover:bg-[#F0EFED]/50 cursor-pointer"
                >
                  {cat.name}
                </button>
              ))}
